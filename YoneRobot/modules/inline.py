...
from YoneRobot import article
from YoneRobot import kiginline
    ...
    inline_help_dicts = [
        {
            ...
            "thumb_urL": "https://telegra.ph/file/8aa1d6210647acafb3572.jpg",
            "keyboard": ".spb ",
        },
        {
...
def inlineinfo(query: str, update: Update, context: CallbackContext) -> None:
    ...
    if user.id == OWNER_ID:
        ...
        text += f"\n\nThis Person is a part of Tsugoku"
        nation_level_present = True
...
def about(query: str, update: Update, context: CallbackContext) -> None:
    ...
    sql.update_user(user.id, user.username)
    about_text = f"""
    Rengoku (@{context.bot.username})
    Maintained by [Rin](t.me/nk_guy)
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    text="Support",
                    url=f"https://t.me/Rengoku",
                ),
                InlineKeyboardButton(
                    text="Channel",
                    url=f"https://t.me/RengokuUpdates",
                ),
    ...
    results.append(
        ...
        InlineQueryResultArticle
            (
            id=str(uuid4()),
            title=f"About Rengoku (@{context.bot.username})",
